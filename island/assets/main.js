document.addEventListener('DOMContentLoaded', function() {
    ...
    var geometry = new THREE.SphereGeometry(1.6, 6, 6);
    ...
    THREE.DefaultLoadingManager.onProgress = function(item, loaded, total) {
        if (total > 15) {
            ...
            if (percent == 1) {
                var message = document.querySelector('#load-message');
                message.setAttribute('n-text', 'text', "Hey there! Loading: 60%");
                setTimeout(function(e) {
                    message.setAttribute('n-text', 'text', "Hey there! Loading: 80%");
                }, 4000);
                setTimeout(function(e) {
                    sim.scene.remove(sphere);
                    message.setAttribute('n-text', 'text', "");
                }, 8000);
            } else {
                var message = document.querySelector('#load-message');
                console.log("Hey there! Loading: " + (percent * 50).toFixed(2) + "%");
                message.setAttribute('n-text', 'text', "Hey there! Loading: " + (percent * 50).toFixed(2) + "%");
            }
        }
    };
    ...
    song.addEventListener("n-sound-loaded", function(e) {
        ...
        if (!firstLoad) {
            ...
            if (playing.getAttribute('material').color != getColorForURL(song.getAttribute('n-sound').src)) {
                ...
                var difference = songLength - currentTimeFromEnd;
                if (paused) {
                    song.components['n-sound'].pauseSound();
                } else {
                    song.components['n-sound'].playSound();
                    song.components['n-sound'].seek(difference);
                }
